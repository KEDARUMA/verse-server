...
describe('Verse.provisionalLogin (integration)', () => {
  ...
  it('provisional account can register a user, that user can login, and can be deleted', async () => {
    ...
    // 1) verifyToken returns payload and ok
    ...
    // 2) refreshToken should fail for provisional token (cannot refresh provisional tokens)
    ...
    // 3) wait 6 seconds so provisional token expires (server provisional tokens expire in 5s)
    await new Promise((r) => setTimeout(r, 6000));

    // 4) verifyToken should now indicate token expired
