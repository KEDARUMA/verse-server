...
## 現状サマリ
# Date: 2025-11-23
- Refresh-token now requires a 5-minute HMAC-signed refresh secret stored per-user and delivered via HttpOnly/Secure/SameSite=Lax cookie, rotated on login/refresh to block indefinite reuse; リフレッシュ用シークレットをユーザごとに5分TTLのHMAC署名トークン＋クッキーで配布し、ログイン/リフレッシュ毎にローテーションして無期限の再利用を防止。
- SigV4 authentication removed from server/client; tests/env cleaned so only TLS+JWT remain for requests; サーバ/クライアントからSigV4認証を撤去しテスト・環境変数も整理、今後のリクエスト認証はTLS+JWTのみ。
- Added optional request debug logging (env DEBUG_REQUEST_LOG) to emit per-request method/path/status/ok/reason for troubleshooting; DEBUG_REQUEST_LOG 環境変数でリクエスト単位の簡易ログ出力を選択可能化（method/path/status/ok/reason）。
...
- 2025-01-26: Implemented refresh-secret cookie flow (5分TTL, HMAC署名, ユーザ別保存・ローテーション) and updated server/login/refresh logic plus tests/env; テストコードを刷新しリフレッシュにはクッキー必須に変更。
- 2025-01-26: Removed SigV4 code/依存/テスト/環境変数からの参照を削除し、TLS+JWTのみの構成に統一。
- 2025-01-26: Added refresh-secret cookie flow (5分TTL) and optional DEBUG_REQUEST_LOG to trace requests during troubleshooting; リフレッシュ用クッキー（5分TTL）とデバッグログ出力フラグを追加。
